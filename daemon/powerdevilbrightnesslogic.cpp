...
float BrightnessLogic::valueAsRatio() const
{
    return static_cast<float>(m_current.value / static_cast<double>(m_current.valueMax));
}

int BrightnessLogic::valueFromRatio(float ratio) const
{
    return qRound(qBound(0.0, static_cast<double>(ratio), 1.0) * m_current.valueMax);
}
...
float BrightnessLogic::ratio(int value) const
{
    return static_cast<float>(value / static_cast<double>(m_current.valueMax));
}
