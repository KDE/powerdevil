...
ActionReply BacklightHelper::brightness(const QVariantMap &args)
{
    ...
    int brightness = m_devices.constFirst().readBrightness();
    if (brightness < 0) {
        return ActionReply::HelperErrorReply();
    }
...
ActionReply BacklightHelper::setbrightness(const QVariantMap &args)
{
    ...
    m_anim.setStartValue(m_devices.constFirst().readBrightness());
...
ActionReply BacklightHelper::brightnessmax(const QVariantMap &args)
{
    ...
    int max_brightness = m_devices.constFirst().readInt("max_brightness"_L1);
    if (max_brightness <= 0) {
        return ActionReply::HelperErrorReply();
    }
