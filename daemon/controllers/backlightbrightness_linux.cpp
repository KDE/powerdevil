...
BacklightBrightness::BacklightBrightness(QList<BacklightSysfsDevice> devices, QObject *parent)
    ...
    // BacklightDetector already checked that LOGIN1_SERVICE is registered on the bus
    , m_login1Interface(new QDBusInterface(LOGIN1_SERVICE, LOGIN1_SESSION_PATH, LOGIN1_SESSION_IFACE, QDBusConnection::systemBus(), this))
{
    if (m_devices.isEmpty()) {
        return;
    }
    m_observedBrightness = m_devices.constFirst().readBrightness();
...
void BacklightBrightness::onDeviceChanged(const UdevQt::Device &device)
{
    ...
    int newBrightness = m_devices.constFirst().readBrightness();
    if (m_observedBrightness == newBrightness || newBrightness == -1) {
        return;
    }
...
void BacklightBrightness::animateBrightnessWithLogin1(int newBrightness)
{
    ...
    int currentBrightness = m_devices.constFirst().readBrightness();
    m_brightnessAnimation->setStartValue(currentBrightness == -1 ? m_observedBrightness : currentBrightness);
    m_brightnessAnimation->setEndValue(newBrightness);
    m_brightnessAnimation->setEasingCurve(currentBrightness < newBrightness ? QEasingCurve::OutQuad : QEasingCurve::InQuad);
...
void BacklightBrightness::setBrightnessWithLogin1(int newBrightness, bool isFinalAnimationValue)
{
    ...
    for (qsizetype i = 0; i < m_devices.size(); ++i) {
        ...
        QDBusPendingCall call = m_login1Interface->asyncCall(u"SetBrightness"_s, device.subsystem, device.interface, perDeviceBrightness);
