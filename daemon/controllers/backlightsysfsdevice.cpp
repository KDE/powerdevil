...
QList<BacklightSysfsDevice> BacklightSysfsDevice::getBacklightTypeDevices()
{
    auto appendIfMaxBrightnessAvailable = [](QList<BacklightSysfsDevice> &list, QString subsystem, QString interface) {
        ...
        int maxBrightness = BacklightSysfsDevice::readInt(syspath, "max_brightness"_L1);
        if (maxBrightness > 0) {
...
// static
int BacklightSysfsDevice::readInt(const QString &syspath, const QLatin1StringView property)
{
    int value = -1;

    QFile file(QString(syspath + u'/' + property));
    if (!file.open(QIODevice::ReadOnly)) {
        qCWarning(POWERDEVIL) << "reading from device " << syspath << "/" << property << " failed with error code " << file.error() << file.errorString();
        return value;
    }
...
int BacklightSysfsDevice::readInt(const QLatin1StringView property) const
{
    return BacklightSysfsDevice::readInt(syspath(), property);
}

int BacklightSysfsDevice::readBrightness() const
{
    int value = readInt("brightness"_L1);
