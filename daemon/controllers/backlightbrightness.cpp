    ...
    , m_maxBrightness(maxBrightness)
{
    ...
    // to "backlight" then). Monitor backlight devices for changes not initiated by this class.
...
void BacklightBrightness::setBrightness(int newBrightness)
{
    ...
    connect(job, &KAuth::ExecuteJob::result, this, [this, job, newBrightness] {
        ...
        // Immediately register the new brightness to everyone while we still animate to it
        m_cachedBrightness = newBrightness;
    });
    job->start();
}
