    ...
    , ui(std::make_unique<Ui::ActionEditWidget>())
{
    ...
    {
        using Enum = PowerDevilProfileSettings::EnumPowerButtonAction;
    ...
    {
        using Enum = PowerDevilProfileSettings::EnumRunScriptPhase;
    ...
    for (auto combo : {ui->kcfg_BluetoothPowerSaving, ui->kcfg_MobileBroadbandPowerSaving, ui->kcfg_WifiPowerSaving}) {
        using Enum = PowerDevilProfileSettings::EnumWifiPowerSaving;
...
{
    m_profilesConfig.setSuspendSessionIdleTimeMsec(minutesToMsec(ui->suspendSessionIdleTimeMsec->value()));
    m_profilesConfig.setDimDisplayIdleTimeMsec(minutesToMsec(ui->dimDisplayIdleTimeMsec->value()));
    m_profilesConfig.setDPMSidleTimeMsec(minutesToMsec(ui->dpmsDisplayIdleTimeMsec->value()));

    if (ui->kernelPowerProfileCombo->isVisible()) {
...
void ActionEditWidget::load()
{
    ...
    ui->dimDisplayIdleTimeMsec->setValue(msecToMinutes(m_profilesConfig.dimDisplayIdleTimeMsec()));
    ui->dpmsDisplayIdleTimeMsec->setValue(msecToMinutes(m_profilesConfig.dPMSidleTimeMsec()));
}
...
void ActionEditWidget::initializeServices()
{
    ...
    QObject::connect(watcher, &QDBusPendingCallWatcher::finished, this, [this](QDBusPendingCallWatcher *watcher) {
        ...
        for (const QString &choice : choices) {
            ui->kernelPowerProfileCombo->addItem(profileNames.value(choice, choice), choice);
        }

        const int idx = qMax(0, ui->kernelPowerProfileCombo->findData(m_profilesConfig.kernelPowerProfile()));
        ui->kernelPowerProfileCombo->setCurrentIndex(idx);
    });
}
