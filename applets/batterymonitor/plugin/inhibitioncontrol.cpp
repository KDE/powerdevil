...
void InhibitionControl::onServiceUnregistered(const QString &serviceName)
{
    if (serviceName == FDO_POWERMANAGEMENT_SERVICE) {
        ...
        m_requestedInhibitions = QList<QVariantMap>();
...
QBindable<QList<QVariantMap>> InhibitionControl::bindableRequestedInhibitions()
{
...
void InhibitionControl::checkInhibitions()
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this](QDBusPendingCallWatcher *watcher) {
        ...
        if (reply.isValid()) {
            ...
            qCDebug(APPLETS::BATTERYMONITOR) << "Failed to retrieve inhibitions:" << reply.error();
        }
        watcher->deleteLater();
    });
}
...
QList<QVariantMap> InhibitionControl::updatedInhibitions(const QList<PolicyAgentInhibition> &inhibitions)
{
    ...
    for (const PolicyAgentInhibition &inhibition : inhibitions) {
        ...
        result.append(QVariantMap{
            {u"appName"_s, inhibition.who},
            {u"prettyName"_s, prettyName},
            {u"icon"_s, icon},
            {u"reason"_s, inhibition.why},
            {u"what"_s, whats},
            {u"active"_s, static_cast<bool>(inhibition.flags & PolicyAgentInhibition::Active)},
            {u"allowed"_s, static_cast<bool>(inhibition.flags & PolicyAgentInhibition::Allowed)},
        });
    }
