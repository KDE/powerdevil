    ...
    compactRepresentation: CompactRepresentation {
        ...
        onWheel: wheel => {
            ...
            if (Math.abs(delta) < 120) {
                // Touchpad scrolling
                brightnessTrackingErrors = screenBrightnessControl.changeBrightnessByDelta((delta/120) * 0.05, brightnessTrackingErrors);
            } else if (wheel.modifiers & Qt.ShiftModifier) {
                // Discrete/wheel scrolling - round to next percentage point
                //newBrightness = Math.round((Math.round(brightness/maxBrightness * 100) + delta/120) / 100 * maxBrightness)
                //newBrightness = Math.round((Math.round(brightness*100/maxBrightness) + delta/120) / 100 * maxBrightness)
                screenBrightnessControl.changeBrightnessBySteps(delta/120, /*step count*/ 100); // TODO
            } else {
                // Discrete/wheel scrolling - round to next step (e.g. 5%, 10%)
                //newBrightness = Math.round(brightness/stepSize + delta/120) * stepSize;
                //newBrightness = Math.round(20*brightness/maxBrightness + delta/120) / 20 * maxBrightness;
                screenBrightnessControl.changeBrightnessBySteps(delta/120, /*step count*/ 20); // TODO (with step rounding)
            }
        }
